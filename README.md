...
| 구성 요소 | 사용자 경험 | 내부 역할 |
| --- | --- | --- |
| **Ainux 도우미(Assist)** | 사용자는 자연어 한 문장만 입력하면 됩니다. `ainux-client assist "패키지 업데이트 해줘"`나 `ainux-client assist "마우스를 오른쪽으로 조금 움직여줘"`처럼 요청하면 계획 수립과 실행 결과를 한 화면에서 확인할 수 있습니다. 필요하면 `--dry-run`으로 미리 검토할 수 있습니다. | 인텐트 파서, 플래너, 안전 점검기, 실행기가 협력하여 계획→실행→피드백 루프를 자동으로 돌립니다. 컨텍스트 패브릭, 스케줄러, 네트워크/하드웨어 조정 도구는 모두 백그라운드에서 호출됩니다. |
...
2. **자연어 요청 제출** – `ainux-client assist "야간에 패키지 업데이트 해줘"`나 `ainux-client assist "마우스를 오른쪽으로 조금 움직여줘"`처럼 한 문장으로 지시하면 됩니다. 기본적으로 실제 실행까지 완료하며, `--dry-run`으로 계획만 확인할 수 있습니다. 명령을 실행하면 CLI가 인텐트 분석→계획 작성→안전 검토→단계 실행까지의 진행 상황을 실시간으로 스트리밍하여 무엇을 하고 있는지 바로 확인할 수 있습니다.
