...
2. **자연어 요청 제출** – `ainux-client assist "야간에 패키지 업데이트 해줘"`, `ainux-client assist "마우스를 오른쪽으로 조금 움직여줘"`, `ainux-client assist "터미널 열어줘"`처럼 한 문장으로 지시하면 됩니다. 기본적으로 실제 실행까지 완료하며, `--dry-run`으로 계획만 확인할 수 있습니다. 명령을 실행하면 CLI가 인텐트 분석→계획 작성→안전 검토→단계 실행→**API 기반 검증**까지의 진행 상황을 실시간으로 스트리밍하여 무엇을 하고 있는지 바로 확인할 수 있습니다. 검증 단계에서 목표가 충족되지 않았다고 판단되면 즉시 추가 계획을 요청합니다.
   *실제로 실행 가능한 대상이 감지되면* 휴리스틱 플래너도 자동으로 `system.launch_application` 단계를 추가하여 터미널·텍스트 편집기 등 데스크톱 앱을 바로 실행합니다. 의도 파악이 다소 애매해도 “execute”, “실행”, “켜줘”와 같은 표현과 함께 앱 이름이 포함되어 있으면 수동 안내 대신 즉시 런처 단계가 생성됩니다.
   마우스 이동·클릭 같은 포인터 제어 요청은 `pyautogui` 모듈이 없는 경우 먼저 `system.ensure_python_package` 단계가 삽입되어 필요한 패키지를 자동 설치한 뒤 실행합니다. 덕분에 추가 안내 없이도 의존성을 충족시키고 바로 자동화를 이어갈 수 있습니다.
