...
2. **자연어 요청 제출** – `ainux-client assist "야간에 패키지 업데이트 해줘"`, `ainux-client assist "마우스를 오른쪽으로 조금 움직여줘"`, `ainux-client assist "터미널 열어줘"`처럼 한 문장으로 지시하면 됩니다. 기본적으로 실제 실행까지 완료하며, `--dry-run`으로 계획만 확인할 수 있습니다. 명령을 실행하면 CLI가 인텐트 분석→계획 작성→안전 검토→단계 실행→**API 기반 검증**까지의 진행 상황을 실시간으로 스트리밍하여 무엇을 하고 있는지 바로 확인할 수 있습니다. 검증 단계에서 목표가 충족되지 않았다고 판단되면 즉시 추가 계획을 요청합니다.
  *실제로 실행 가능한 대상이 감지되면* 휴리스틱 플래너도 자동으로 `system.launch_application` 단계를 추가하여 터미널·텍스트 편집기 등 데스크톱 앱을 바로 실행합니다. 의도 파악이 다소 애매해도 “execute”, “실행”, “켜줘”와 같은 표현과 함께 앱 이름이 포함되어 있으면 수동 안내 대신 즉시 런처 단계가 생성됩니다.
  마우스 이동·클릭 같은 포인터 제어 요청은 `pyautogui` 모듈이 없는 경우 먼저 `system.ensure_python_package` 단계가 삽입되어 필요한 패키지를 자동 설치한 뒤 실행합니다. 동일한 메커니즘이 모든 단계에 적용되므로, 실행 로그에서 “requires the '…' package”나 `ModuleNotFoundError`와 같은 메시지가 감지되면 도우미가 즉시 패키지 설치 단계를 재계획에 추가하고 재시도합니다. 여기에 더해 “Command not found: firefox”처럼 실행 파일이 없다는 메시지가 나오면 `apt install <패키지>` 명령을 자동으로 앞에 삽입해 데스크톱 애플리케이션까지 설치한 뒤 다시 실행합니다. 덕분에 추가 안내 없이도 의존성을 충족시키고 바로 자동화를 이어갈 수 있습니다.
...
또한 “어셈블리 코드 실행해줘”처럼 저수준 제어를 원하는 경우에는 두 단계가 자동으로 이어집니다.

1. 먼저 `system.inspect_command`가 요청에 등장한 실행 파일을 추정해 `which`, `type`, `ls -l`, `file` 등을 실행하며 실존 경로와 패키지 메타데이터를 수집합니다. 사용자가 단순히 “execute something”처럼 프로그램 이름만 말한 경우에도 이 단계가 자동으로 후보를 탐색해 실행에 필요한 정보를 확보합니다.
2. 수집된 정보는 즉시 검증기/플래너로 되돌려 보내져 `system.execute_low_level` 단계에서 사용할 런처 코드를 합성합니다. 실행 파일 경로가 확정되면 임시 디렉터리에 x86_64용 어셈블리(또는 필요 시 C) 스텁을 만들어 컴파일하고, 사용자가 별도의 소스를 제공하지 않아도 OS 수준에서 직접 실행합니다. 실행 단계에서 소스가 비어 있어도 실행기가 동일한 추론을 반복하므로, 검증 루프에서 추가로 생성된 단계 역시 자동으로 동작합니다.

모든 실행 결과는 CLI 관찰자에 실시간으로 스트리밍되므로 어떤 단계가 실제로 실행되었는지 바로 확인할 수 있습니다.
