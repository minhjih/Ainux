...
2. **자연어 요청 제출** – `ainux-client assist "야간에 패키지 업데이트 해줘"`나 `ainux-client assist "마우스를 오른쪽으로 조금 움직여줘"`처럼 한 문장으로 지시하면 됩니다. 기본적으로 실제 실행까지 완료하며, `--dry-run`으로 계획만 확인할 수 있습니다.
