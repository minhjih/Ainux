...
또한 “어셈블리 코드 실행해줘”처럼 저수준 제어를 원하는 경우 `system.execute_low_level` 기능이 임시 디렉터리에 C/어셈블리 코드를 컴파일하거나, 16진수로 제공된 기계어를 바이너리로 저장해 직접 실행합니다. 사용자가 “assembly로 firefox 실행해줘”처럼 대상 프로그램만 언급해도 플래너가 자동으로 실행 파일 경로를 찾고 x86_64용 어셈블리 런처(필요 시 C 스텁)를 생성해 컴파일합니다. 실행 단계에서 소스가 비어 있어도 실행기가 동일한 추론을 반복해 런처 코드를 합성하므로, 계획 재검토 과정에서 추가된 단계 역시 자동으로 OS 수준 제어를 수행합니다. 모든 실행 결과는 CLI 관찰자에 실시간으로 스트리밍되므로 어떤 단계가 실제로 실행되었는지 바로 확인할 수 있습니다.
