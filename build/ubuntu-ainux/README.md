...
> 반복되면 `ftp.kaist.ac.kr/ubuntu-ports`, `mirror.kakao.com/ubuntu-ports`, `ftp.harukasan.org/ubuntu-ports`
> 등의 ports 전용 미러로 자동 재시도하며, 성공한 미러와 후보 목록은 `/etc/apt/ainux-mirrors`
> 에 저장됩니다. 이후 패키지 설치 단계에서는 빌더가 배치한 APT 폴백 헬퍼가 이 목록을 순회하면서
> 미러를 자동으로 교체하므로, 특정 미러에서 `Connection timed out`이 반복돼도 다음 후보로
> 자연스럽게 넘어갑니다. 특정 미러만 사용하고 싶다면 `--mirror` 옵션으로 명시하세요.
...
> 🔁 **APT 미러 폴백 헬퍼:** `/usr/local/lib/ainux/apt-failover.sh`가 chroot에 포함되어
> `install_packages.sh`와 라이브 부트 구성 단계가 이를 호출합니다. 패키지 다운로드가 실패하면
> 헬퍼가 다음 후보 미러로 `sources.list`를 교체하고 다시 시도하므로, `gnome-control-center-data`
> 처럼 덩치 큰 패키지가 특정 미러에서만 느리게 내려올 때도 빌드가 끝까지 진행됩니다.
